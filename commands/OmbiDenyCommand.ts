...
export class OmbiDenyCommand implements ISlashCommand {
  public command = 'ombi-deny';
  public i18nParamsExample = 'slashcommand_deny_params';
  public i18nDescription = 'slashcommand_deny_description';
  ...
  public async executor(context: SlashCommandContext, read: IRead, modify: IModify, http: IHttp, persis: IPersistence): Promise<void> {
    ...
    const requestResponse = await http.put(url, {
      headers,
      data,
    });
    ...
    if (requestResponse.statusCode === 200 && requestResponse.content && requestResponse.content) {
      ...
      let message = (jsonResult && jsonResult.message) ? jsonResult.message : (typeForReturnMessage + ' ' + id + ' has been successfully denied!');
      if (message.indexOf('deleted') !== -1) {
        message.replace('deleted', 'denied'); // Message comes back 'deleted'...
      }
      let color = '#00CE00';
